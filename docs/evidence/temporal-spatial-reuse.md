...
This historical validation asked whether a weight-free Observation path could build spatial state from information-prior regions, reuse that state across a continuous shot, and rebuild after a scene transition at materially lower cost than eager per-frame processing.
...
- event-driven path: sparse validation opportunities, coarse structural watchdog, sparse anchor tracking, structural recovery only after cheaper evidence failed, and full information-prior rebuild only when spatial evidence failed
- no OCR recognition
...
D4  rebuild information-prior regions and spatial state after evidence failure
```
...
- 66 continuous-shot frames produced zero full information rebuilds;
...
The replay supports sparse validation plus event-driven rebuild as a low-cost spatial-state maintenance direction on this sample. It does **not** establish deployment performance, universal region quality, or production thresholds. Decode, scheduler contention, memory pressure and target-device transforms were outside the timing boundary.

It also preserves the runtime ownership rule: Observation may construct evidence and propose what is worth observing, while the existing runtime remains responsible for task creation, admission, scheduling and merge.
